...
sub neighbors_view {
    ...
    if ( $FORM{TREE_ID} ) {
        ...
        my $stbl = nms_snmp_table(
            { ID => $FORM{TREE_ID}, OID => 'lldpRemTable', HASH => 1 } );
        my %remp;
        foreach my $key ( keys %$stbl ) {
            #     $stbl->{$key}->{lldpRemPortId} =~ s|1/||;
            $remp{ $stbl->{$key}->{lldpRemLocalPortNum} } = [
                $stbl->{$key}->{lldpRemSysName},
                $stbl->{$key}->{lldpRemPortId}
            ];
        }
        my $matbl = nms_snmp_table(
            { ID => $FORM{TREE_ID}, OID => 'lldpRemManAddrTable', HASH => 1 } );
        my %bd;
        my %outputs;
        my %inputs;
        my %links;
        ...
        foreach my $key ( keys %$matbl ) {
            $inputs{ $matbl->{$key}->{lldpRemLocalPortNum} } = (
                {
                    label => "$lang{PORT} $matbl->{$key}->{lldpRemLocalPortNum}"
                }
            );
        }
    ...
    my $tree = $html->element(
        'div',
        $search . neighbors_tree(),
        {
            class => 'col-md-4',
...
sub neighbors_tree {
    ...
    return make_tree(
        { core => { data => \@lldp_tree }, plugins => [ 'search', 'sort' ] } );
}
